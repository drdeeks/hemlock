...
# Directories to always skip (node_modules, caches, vcs)
SKIP_DIRS = {
...
def is_excluded(path):
    ...
    # check each prefix in the path against exclude set
...
def extract_version_score(name: str):
    # match v1, v2.0, V_3.1.4, -v2 only when preceded by separator or at start
...
def scan(root, db):
    ...
    seen_real = set()  # prevent symlink loops
        ...
        if entry == TARGET_DIR and os.path.islink(full_entry):
            try:
                ...
                if os.path.isdir(resolved) and resolved not in seen_real:
                    # inject as a synthetic walk target
    ...
    for base, dirs, files in os.walk(root, followlinks=False):
        # prune excluded + heavy dirs in-place so os.walk doesn't descend
        dirs[:] = [
            d for d in dirs
        ...
        for sub, subdirs, fs in os.walk(base, followlinks=False):
            ...
            for f in fs:
                ...
                # deduplicate by real path (handles symlinked dirs)
                try:
                    real = os.path.realpath(full)
                    if real in seen_real:
                        continue
...
def match(row, terms):
    ...
    for t in terms:
        ...
        if t.startswith("."):
            ...
            # fuzzy: name or path contains term
...
def diff(db):
    ...
    # detect modified: same path, different hash
    modified = []
