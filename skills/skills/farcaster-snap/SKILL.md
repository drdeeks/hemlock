---
name: farcaster-snap
description: Use this skill whenever the users wants to generate a Farcaster embedded
  app (aka snap), deploy an app to production, or edit an existing app. Activate when
  the user mentions snaps, embedded apps, interactive casts, or cast apps.
version: 1.0.0
author: openclaw
license: MIT
---
...
## Build Gotchas (learned from Roast or Toast)

### TypeScript for Neynar Hosting
- `noEmit: false` in tsconfig — `true` blocks dist output
- `rootDir: "src"` — `.` nests output at `dist/src/index.js`
- Exclude `src/server.ts` from compilation (Edge runtime, not Node)
- `skipLibCheck: true` — zod type conflicts
- All relative imports need `.js` extension with `moduleResolution: "NodeNext"`

### Effects Type Must Be Literal
```ts
// WRONG — TypeScript infers string[]
const effects = isRoast ? [] : ["confetti"];

// RIGHT — explicit literal type
const effects: "confetti"[] = isRoast ? [] : ["confetti"];
```

### Max 7 Root Children
The `page` root element can only have 7 direct children. Nest extra elements in sub-stacks:
```ts
// 9 elements? Nest them:
page: { children: ["art", "header", "result", "btns"] }
header: { children: ["em", "title", "target"] }  // nested, doesn't count
btns: { children: ["actionRow", "mintBtn"] }       // nested
```

### All Return Paths Must Have Same Shape
If some pages have `effects` and others don't, TypeScript can't reconcile the union type. Always include `effects` (even empty) on all return paths.

### SVG Images for NFT Minting
Server-side SVG generation works well for snap-linked mint pages. No external image API needed. Use `image/svg+xml` content type. Can be used as OG/frame images too.
