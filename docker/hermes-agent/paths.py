...
Environment Variables:
    ...
    HERMES_BACKUPS:   Backups directory
...
class PathResolver:
    ...
    def agents_dir(self) -> Path:
        ...
        if 'agents_dir' not in self._cache:
            default = '/agents' if self._is_docker else str(self._root / 'agents')
    ...
    def crews_dir(self) -> Path:
        ...
        if 'crews_dir' not in self._cache:
            default = '/crews' if self._is_docker else str(self._root / 'crews')
    ...
    def logs_dir(self) -> Path:
        ...
        if 'logs_dir' not in self._cache:
            default = '/var/log/openclaw' if self._is_docker else str(self._root / 'logs')
    ...
    def memory_dir(self) -> Path:
        ...
        if 'memory_dir' not in self._cache:
            default = '/runtime/memory' if self._is_docker else str(self._root / 'memory')
    ...
    def backups_dir(self) -> Path:
        ...
        if 'backups_dir' not in self._cache:
            default = '/backups' if self._is_docker else str(self._root / 'backups')
    ...
    def config_dir(self) -> Path:
        ...
        if 'config_dir' not in self._cache:
            default = '/etc/openclaw' if self._is_docker else str(self._root / 'config')
    ...
    def ensure_dirs(self, *names: str) -> None:
        ...
        if not names:
            names = (
                ...
                'config_dir', 'scripts_dir', 'models_dir',
            )
        for name in names:
