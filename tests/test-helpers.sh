...
# Aliases used by security/system-integration tests (map to success/error)
pass() {
    echo -e "${GREEN}[PASS]${NC} $1"
    return 0
}
...
# Assert string contains substring (supports \| as OR separator for grep -E)
assert_contains() {
    local string="$1"
    local substring="$2"
    local message="$3"
    
    # Use grep -qE with | as OR operator (convert \| to |)
