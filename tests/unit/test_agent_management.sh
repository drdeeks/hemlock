...
cleanup() {
    ...
    if [[ -d "$TEST_AGENT_DIR" ]]; then
        cd "$RUNTIME_ROOT"
        ./runtime.sh agent-stop "$TEST_AGENT" 2>/dev/null || true
...
cd "$RUNTIME_ROOT"
output=$(./runtime.sh agent-logs "$TEST_AGENT" 2>&1 || true)
...
cd "$RUNTIME_ROOT"
output=$(./runtime.sh agent-stop "$TEST_AGENT" 2>&1 || true)
...
# Create agent
create_output=$(./runtime.sh create-agent "$TEST_AGENT" --model gpt-4 --force 2>&1 || true)
if [[ ! -d "$TEST_AGENT_DIR" ]]; then
...
# Monitor agent
monitor_output=$(timeout 2 ./runtime.sh agent-monitor 2>&1 || true)
echo "  Agent monitor attempted"

# Stop agent
stop_output=$(./runtime.sh agent-stop "$TEST_AGENT" 2>&1 || true)
echo "  Agent stop attempted"

# Check logs
logs_output=$(./runtime.sh agent-logs "$TEST_AGENT" 2>&1 || true)
